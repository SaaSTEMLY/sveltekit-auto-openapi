    ...
    async load(id) {
      if (id === RESOLVED_VIRTUAL_SCHEMA_MODULE_ID) {
        try {
          ...
          const { openApiPaths } = await generate(null, root);
          cachedSchema = openApiPaths;
          return `export default ${JSON.stringify(cachedSchema, null, 2)};`;
        } catch (err) {
      ...
      if (id === RESOLVED_VIRTUAL_VALIDATION_MAP_MODULE_ID) {
        try {
          ...
          for (const methods of Object.values(validationMap)) {
            for (const data of Object.values(methods as Record<string, any>)) {
...
`;
    ...
    async closeBundle() {
      try {
        ...
        await fs.promises.writeFile(validationMapPath, validationMapContent);
      } catch (err) {
